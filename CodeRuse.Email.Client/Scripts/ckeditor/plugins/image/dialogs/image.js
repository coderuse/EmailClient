    ...
    var imageDialog = function (editor, dialogType) {
        ...
        return {
            ...
            onHide: function () {
                ...
                if (this.uploadEl) {
                    this.uploadEl.removeStyle('display');
                }

                if (this.urlEl) {
                    this.urlEl.setStyle('display', 'none');
                }
                    ...
                    children: [
                        {
                            type: 'hbox',
                            align: 'left',
                            className: 'cke_dialog_image_upload_type',
                            children: [
                                {
                                    type: 'radio',
                                    id: 'upload-type',
                                    label: 'Which type of uploader do you want to use?',
                                    items: [['URL', 1], ['Local', 2]],
                                    'default': 2,
                                    onChange: function () {
                                        var dialog = this.getDialog();
                                        var value = this.getValue();
                                        if (!dialog.urlEl) {
                                            dialog.urlEl = CKEDITOR.document.getById(dialog.getContentElement('info', 'txtUrl').domId);
                                        }
                                        if (!dialog.uploadEl) {
                                            dialog.uploadEl = CKEDITOR.document.getById(dialog.getContentElement('info', 'upload').domId);
                                        }
                                        switch (value) {
                                            case '1':
                                                dialog.urlEl.removeStyle('display');
                                                dialog.uploadEl.setStyle('display', 'none');
                                                break;
                                            case '2':
                                                dialog.uploadEl.removeStyle('display');
                                                dialog.urlEl.setStyle('display', 'none');
                                                break;
                                            default:
                                                break;
                                        }
                        ...
                        },
                        {
                            type: 'hbox',
                            align: 'left',
                            className: 'cke_dialog_image_upload',
                            children: [
                                {
                                    type: 'file',
                                    id: 'upload',
                                    ...
                                    onChange: function () {
                                        ...
                                        var selectedFile = inputFiles && inputFiles.length > 0 ? inputFiles[0] : undefined;
                                        var dialog = this.getDialog();
                                        var original = dialog.originalElement;
                                        if (!selectedFile) {
                                            dialog.preview.removeAttribute('src');
                                    ...
                                    commit: function (type, element) {
                                        if (type == IMAGE) {
                                            var dialog = this.getDialog();
                                            var original = dialog.originalElement;
                                            element.data('cke-saved-src', original.$.src);
                        ...
                        children: [{
                            ...
                            html: '<div>' + CKEDITOR.tools.htmlEncode(editor.lang.common.preview) + '<br>' +
                                ...
                                '<div class="ImagePreviewBox"><table><tr><td>' +
                                    ...
                                    '<img id="' + previewImageId + '" alt="" /></a><br/>' +
